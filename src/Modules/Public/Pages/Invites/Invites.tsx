const Invites = () => {
	...
	return (
		<div className="w-full h-screen m-0 md:ml-2 p-1 bg-white overflow-y-scroll scrollbar-hide">
			<div className="w-12/12 h-auto mx-auto p-2 my-5  flex flex-row rounded-sm shadow-md items-center">
				<img
					src="/2.png"
					alt="user image"
					className="w-24 h-24 rounded-full object-cover items-center"
				/>
				<h2 className="text-2xl md:text-4xl text-blue-700 font-roboto font-bold">My Invites</h2>
			</div>
			...
			<div className="w-full h-auto m-3 p-5 my-5 flex flex-row flex-wrap  items-center border-l-4 border-slate-400 ">
				{InvitedUsers &&
					InvitedUsers.map((i, n) => (
						<div
							className="w-52 h-auto mx-auto my-5  flex flex-col items-center cursor-pointer"
							key={n}
						>
							<img
								src={i.img}
							...
							<div className="w-full m-0 p-2 py-3 bg-white rounded-sm shadow-lg hover:shadow-lg">
								<h2 className="text-md  truncate text-blue-800 font-roboto">{i.name}</h2>
			...
			<div className="w-full h-20 md:h-24 p-4 py-3 fixed bottom-0 flex flex-row border-t-2 border-slate-200 items-center  bg-white">
				<h2 className="text-3xl md:text-4xl text-navDarkBlue font-roboto ">Total: $500</h2>
			</div>
		</div>
	);
};
